...
pureSolve (nums, boards) = fromJust ans
  where
    f called boards = case bingos of
      [] -> Nothing
      _ -> Just (called, head bingos)
      where
        bingos = filter (isBingo called) boards
    turns = map (\c -> take c nums) [1 ..]
    noBingos = map (\nums -> filter (not . isBingo nums) boards) turns
    lastBoard = head $ fromJust $ find (\bs -> (length bs) == 1) noBingos
    Just ans = find isJust $ map (\called -> f called [lastBoard]) turns
